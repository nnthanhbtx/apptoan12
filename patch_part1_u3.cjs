...
const target = `const uMatch = fullBlockText.match(/\\[U\\]\\s*([A-D])[\\.\\)]?\\s*.*\\[\\/U\\]/i) || fullBlockText.match(/\\[U\\]\\s*([A-D])[\\.\\)]?\\s*\\[\\/U\\]/i);`;

const replacement = `let uMatch = fullBlockText.match(/\\[U\\]\\s*([A-D])[\\.\\)]?\\s*.*\\[\\/U\\]/i) || fullBlockText.match(/\\[U\\]\\s*([A-D])[\\.\\)]?\\s*\\[\\/U\\]/i);
      if (!uMatch) {
